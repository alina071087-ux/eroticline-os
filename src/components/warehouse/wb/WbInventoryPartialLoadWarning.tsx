...
type WbInventoryPartialLoadWarningProps = {
  show: boolean;
};

export function WbInventoryPartialLoadWarning({
  show,
}: WbInventoryPartialLoadWarningProps) {
  if (!show) {
    return null;
  }
    ...
    <section className="rounded-2xl border border-red-500/20 bg-red-500/10 px-4 py-3 shadow-[0_1px_2px_rgba(0,0,0,0.35)]">
      <div className="flex items-start gap-3">
        ...
        <p className="text-sm leading-relaxed text-red-100/90">
          Загружена только часть остатков Wildberries
        </p>
      </div>
    </section>
  );
