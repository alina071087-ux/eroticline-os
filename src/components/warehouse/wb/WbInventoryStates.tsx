...
type WbInventoryLoadingProps = {
  message?: string;
};

export function WbInventoryLoading({
  message = "Загрузка остатков Wildberries…",
}: WbInventoryLoadingProps) {
  return (
...
type WbInventoryErrorProps = {
  message: string;
  onRetry: () => void;
};

export function WbInventoryError({ message, onRetry }: WbInventoryErrorProps) {
  return (
...
export function WbInventoryEmpty() {
  return (
    <section className="flex min-h-[280px] flex-col items-center justify-center rounded-2xl border border-white/[0.06] bg-[#111113] px-6 py-12 text-center shadow-[0_1px_2px_rgba(0,0,0,0.35),0_8px_24px_rgba(0,0,0,0.2)]">
