...
type OzonInventoryLoadingProps = {
  message?: string;
};

export function OzonInventoryLoading({
  message = "Загрузка остатков Ozon…",
}: OzonInventoryLoadingProps) {
  return (
...
type OzonInventoryErrorProps = {
  message: string;
  onRetry: () => void;
};

export function OzonInventoryError({ message, onRetry }: OzonInventoryErrorProps) {
  return (
...
export function OzonInventoryEmpty() {
  return (
    <section className="flex min-h-[280px] flex-col items-center justify-center rounded-2xl border border-white/[0.06] bg-[#111113] px-6 py-12 text-center shadow-[0_1px_2px_rgba(0,0,0,0.35),0_8px_24px_rgba(0,0,0,0.2)]">
      <PackageOpen className="h-8 w-8 text-zinc-500" strokeWidth={1.75} />
      <p className="mt-4 text-sm text-zinc-400">
        По выбранным фильтрам остатки не найдены.
      </p>
    </section>
  );
}
